...
public class Plots_FB_Plant1_Water extends AppCompatActivity {
    private static final String TAG = "Plots_FB_Plant1_Water";
    ...
    private UserInformation_FB_Plant1 user;

    ArrayList<XyValuesPlant1> xyValuesArrayPlant1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_plots__fb__plant1__water);

        user = new UserInformation_FB_Plant1();

        databaseFB = FirebaseDatabase.getInstance();
        databaseReference = databaseFB.getReference().child("Bevattningar").child("Plant1");

        SensorPlot = (GraphView)findViewById(R.id.plots_fb_plant1_water_graphView);
        ...
        xyValuesArrayPlant1 = new ArrayList<>();
/**
 double start = -10000;
 double end = 10000;
 for(int i = 0; i<40; i++) {
 ...
 */
     ...
     //   xyValuesArrayPlant1 = sortArray(xyValuesArrayPlant1);
/**
 ...
 */
                ...
                new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        for(DataSnapshot ds: dataSnapshot.getChildren()){
                            ...
                            user = ds.getValue(UserInformation_FB_Plant1.class);
                            //list.add("Planta nr: " + user.getPlant().toString() + "  Vattenmängd: " + user.getWater_amount().toString() + "    Tid: " + user.getTime_hour().toString() + ":" + user.getTime_min().toString());
                            displayWater = Integer.parseInt(user.getWater_amount_plant1().toString());
                            displayHour = Integer.parseInt(user.getTime_hour_plant1().toString());
                            displayMin = Integer.parseInt(user.getTime_min_plant1().toString());
                            displayPlant = Integer.parseInt(user.getPlant1().toString());
                            ...
                            Toast.makeText(Plots_FB_Plant1_Water.this, "STR " + displayDayStr, Toast.LENGTH_SHORT).show();
                            Toast.makeText(Plots_FB_Plant1_Water.this, "INT " + displayDay, Toast.LENGTH_SHORT).show();
                            //displayMin = xyValuesArrayTest.get(Integer.parseInt(user.getTime_min().toString())).getX();
                            if(displayPlant == 1) {
    ...
    int appendGraphValues(int x, int y, int i){
        ...
        if(x != 0 || y != 0){
            for (int j = i; j <= i; j++) {
                xyValuesArrayPlant1.add(new XyValuesPlant1(x, y));
                comparer = i;
            }
            xyValuesArrayPlant1 = sortArray(xyValuesArrayPlant1);

            int arraySize = xyValuesArrayPlant1.size();
 ...
 */
    ...
    private void drawGraph(ArrayList<XyValuesPlant1> ArrayPlant1){
        ...
        for(int l = 0; l < arraySize; l++) {
            ...
            Toast.makeText(this, "" + l, Toast.LENGTH_SHORT).show();
            double x = ArrayPlant1.get(l).getX_plant1();
            double y = ArrayPlant1.get(l).getY_plant1();
            xySeries.appendData(new DataPoint(x, y), true, 1000);
    ...
    private ArrayList<XyValuesPlant1> sortArray(ArrayList<XyValuesPlant1> arrayTest){
        ...
        while(true){
            ...
            try {
                 ...
                 */
                double tempY = arrayTest.get(m - 1).getY_plant1();
                double tempX = arrayTest.get(m - 1).getX_plant1();
                if(tempX > arrayTest.get(m).getX_plant1()){
                    arrayTest.get(m - 1).setY_plant1(arrayTest.get(m).getY_plant1());
                    arrayTest.get(m).setY_plant1(tempY);
                    arrayTest.get(m - 1).setX_plant1(arrayTest.get(m).getX_plant1());
                    arrayTest.get(m).setX_plant1(tempX);
                }
                else if(tempY == arrayTest.get(m).getY_plant1()){

                    count++;
                    Log.d(TAG, "sortArray: count = " + count);
                }
                else if(arrayTest.get(m).getX_plant1() > arrayTest.get(m-1).getX_plant1()){
