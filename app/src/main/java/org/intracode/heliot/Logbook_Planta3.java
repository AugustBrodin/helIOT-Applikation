...
public class Logbook_Planta3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_logbook__planta3);

        database = FirebaseDatabase.getInstance();

        databaseReference = database.getReference().child("Logbook").child("Plant3");

        userInformation_FB_Plant3 = new UserInformation_FB_Plant3();

        listView = (ListView) findViewById(R.id.logBook_lisView_Plant3);

        list = new ArrayList<>();
        adapter = new ArrayAdapter<String>(this, R.layout.logbook_planta3_info, R.id.logbook_planta3_info_text, list);
                ...
                new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        for (DataSnapshot ds : dataSnapshot.getChildren()) {
                            ...
                            account_Counter = Integer.parseInt(userInformation_FB_Plant3.getAccount_comperer_plant3());
                            String getMessage = userInformation_FB_Plant3.getMessage_plant3().toString();
                            String getTimeStamp = userInformation_FB_Plant3.getTimestamp_plant3().toString();
                            //
                            ...
                            if(databaseCounter < 9) {
                                ...
                                for (int m = 2; m < countedValues; m++) {
                                    ...
                                    if ((databaseValues[m - 1] - databaseValues[m - 2]) > 1 && databaseValues[m - 2] == 1) {
                                        ...
                                        Toast.makeText(Logbook_Planta3.this, "THIS SUCKS", Toast.LENGTH_SHORT).show();
                                        ...
                                        Toast.makeText(Logbook_Planta3.this, "AWESOME", Toast.LENGTH_SHORT).show();
                                        list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                    } else if ((databaseValues[m - 1] - databaseValues[m - 2]) == 1 && databaseValues[m] == 0 && databaseValues[m - 2] != 1 && databaseValues[0] != 1) {
                                        Toast.makeText(Logbook_Planta3.this, "AWESOME111", Toast.LENGTH_SHORT).show();
                                        list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                    } else if ((databaseValues[m - 1] - databaseValues[m - 2]) == 1 && databaseValues[m] == 0 && databaseValues[m - 2] == 1 && databaseValues[0] == 1) {
                                        Toast.makeText(Logbook_Planta3.this, "AWESOME112", Toast.LENGTH_SHORT).show();
                                        list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                    } else if ((databaseValues[m - 1] - databaseValues[m - 2]) == 1 && databaseValues[m] == 0 && databaseValues[m-2] != 1 && databaseValues[0] == 1) {
                                        Toast.makeText(Logbook_Planta3.this, "AWESOME113", Toast.LENGTH_SHORT).show();
                                        list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                        //VERIFIED WORKS
                                    }else if ((databaseValues[m - 1] - databaseValues[m - 2]) == 1 && (databaseValues[m] - databaseValues[m - 1]) > 1 && databaseValues[0] != 1) {
                                        Toast.makeText(Logbook_Planta3.this, "AWESOME22", Toast.LENGTH_SHORT).show();
                                        list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                    }
                                    //
                                    // VERIFIED WORKS
                                    else if ((databaseValues[m - 1] - databaseValues[m - 2]) == 1 && (databaseValues[m] - databaseValues[m - 1]) > 1 && databaseValues[0] == 1) {
                                        Toast.makeText(Logbook_Planta3.this, "AWESOME33", Toast.LENGTH_SHORT).show();
                                        list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                    }
                                    //
                                }
                                ...
                                if (databaseValues[0] == 1 && databaseValues[1] == 0) {
                                    Toast.makeText(Logbook_Planta3.this, "DUDESTAR", Toast.LENGTH_SHORT).show();
                                    list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                } else if (databaseValues[0] != 0 && databaseValues[1] == 0) {
                                    Toast.makeText(Logbook_Planta3.this, "DUDESTAR", Toast.LENGTH_SHORT).show();
                                    list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                } else if (databaseValues[0] == 0 && databaseValues[1] == 0) {
                                    Toast.makeText(Logbook_Planta3.this, "DUDEFAIL", Toast.LENGTH_SHORT).show();
                                    list.add("*" + databaseValuesMessage[databaseCounter] + "\n" + databaseValuesTimeStamp[databaseCounter]);
                                }
                            }
                            else{
                                Toast.makeText(Logbook_Planta3.this, "Loggbok är vid tidpunkten full.", Toast.LENGTH_LONG).show();
                            }
                        }
